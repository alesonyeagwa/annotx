...
function webViewerTextLayerRendered(evt) {
  var event = document.createEvent('CustomEvent');
  event.initCustomEvent('text-rendered', true, true, evt);
  document.dispatchEvent(event);
}
...
function webViewerRotateCw() {
  ...
  var event = document.createEvent('CustomEvent');
  event.initCustomEvent('page-rotatecw', true, true, {});
  document.dispatchEvent(event);
}
...
function webViewerRotateCcw() {
  PDFViewerApplication.rotatePages(-90);
  var event = document.createEvent('CustomEvent');
  event.initCustomEvent('page-rotateccw', true, true, {});
  document.dispatchEvent(event);
}
...
function () {
  ...
  _createClass(PDFSidebar, [{
    ...
    value: function _addEventListeners() {
      ...
      this.annotationsButton.addEventListener('click', function () {
        _this3.switchView(SidebarView.ANNOTATIONS);
      });
...
function () {
  function Toolbar(options, eventBus) {
    ...
    this._bindListeners();
    this._bindCursorToolsListener(this.items);
  }
  ...
  _createClass(Toolbar, [{
    ...
    value: function _bindListeners() {
      ...
      items.cursorSelectToolButton.addEventListener('click', function(){
        eventBus.dispatch('switchcursortool', {
          source: self,
          tool: _pdf_cursor_tools.CursorTool.SELECT
        });
      });
    ...
    key: "_bindCursorToolsListener",
    value: function _bindCursorToolsListener(items) {
      this.eventBus.on('cursortoolchanged', function (_ref) {
        var tool = _ref.tool;
        items.cursorSelectToolButton.classList.toggle('toggled', tool === _pdf_cursor_tools.CursorTool.SELECT);
        items.cursorHandToolButton.classList.toggle('toggled', tool === _pdf_cursor_tools.CursorTool.HAND);
      });
    }
  }, {
    key: "_localized",
